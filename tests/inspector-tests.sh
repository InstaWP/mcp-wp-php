...
run_test() {
    ...
    if eval "$command" > /tmp/mcp_test_output 2>&1; then
...
CREATED_POST_ID=$(cat /tmp/mcp_test_output | grep -o '"id": [0-9]*' | head -1 | grep -o '[0-9]*')
...
CREATED_TERM_ID=$(cat /tmp/mcp_test_output | grep -o '"id": [0-9]*' | head -1 | grep -o '[0-9]*')
...
if [ -n "$CREATED_TERM_ID" ]; then
    ...
    run_test "Delete term" \
        "$INSPECTOR --method tools/call --tool-name delete_term --tool-arg taxonomy=category --tool-arg term_id=$CREATED_TERM_ID | grep -q '\"deleted\": true'"
else
